...
public class ExceptionHandlerAdvice {
    ...
    protected ResponseEntity<?> handleBaseException(BaseException e) {
        return ResponseEntity.status(e.getBaseResponseCode().getStatus())
            .body(BaseResponse.error(e.getBaseResponseCode().getStatus().value(),
                e.getBaseResponseCode().getCode(), e.getBaseResponseCode().getMessage()));
    }
    ...
    protected ResponseEntity<?> handleMethodArgumentNotValidException(
        ...
        BaseResponseCode baseResponseCode = BaseResponseCode.findByCode(
            e.getFieldError().getDefaultMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            .body(
                BaseResponse.error(baseResponseCode.getStatus().value(), baseResponseCode.getCode(),
